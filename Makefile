...
.PHONY: all clean test unit integration verify bptree-contract benchmark benchmark-run gen-small gen-large verify-small verify-large verify-fail-id verify-fail-age verify-fail-score verify-fail-columns verify-fail-count
...
tests/unit/test_bptree_contract: tests/unit/test_bptree_contract.c tests/support/unity.c
	$(CC) $(CFLAGS) -Itests/support -o $@ tests/unit/test_bptree_contract.c tests/support/unity.c
...
integration: $(TARGET)
	...
	cmp -s "$$TMP_DB/select_primary_key.out" tests/integration/select_primary_key.expected
	TMP_DB=$$(mktemp -d /tmp/sql-parser-integration-XXXXXX); \
	mkdir -p "$$TMP_DB/schema" "$$TMP_DB/tables"; \
	cp tests/fixtures/db/schema/users.schema "$$TMP_DB/schema/users.schema"; \
	cp tests/fixtures/db/tables/users.csv "$$TMP_DB/tables/users.csv"; \
	./$(TARGET) --sql tests/integration/insert_then_select_primary_key.sql --db "$$TMP_DB" > "$$TMP_DB/insert_then_select_primary_key.out"; \
	cmp -s "$$TMP_DB/insert_then_select_primary_key.out" tests/integration/insert_then_select_primary_key.expected

test: unit integration

clean:
	rm -f $(TARGET) $(UNIT_TEST_BINS) $(VERIFY_BINS) $(BPTREE_TEST_BINS) $(BENCH_BINS)
