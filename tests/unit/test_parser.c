...
static void test_parse_insert_followed_by_select(void) {
    ...
    ok = tokenize_sql(float_sql, &float_tokens, &error);
    TEST_ASSERT_TRUE_MESSAGE(ok, error.message);
    ok = parse_queries(&float_tokens, &float_queries, &error);
    TEST_ASSERT_TRUE_MESSAGE(ok, error.message);

    TEST_ASSERT_EQUAL_INT(1, float_queries.count);
    TEST_ASSERT_TRUE(float_queries.items[0]->select_query.has_where);
    TEST_ASSERT_EQUAL_INT(VALUE_FLOAT, float_queries.items[0]->select_query.where.value.type);
    TEST_ASSERT_EQUAL_STRING("3.75", float_queries.items[0]->select_query.where.value.raw);
